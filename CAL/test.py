...
@torch.no_grad()
def extract_vid_feature(logger, model, dataloader, vid2clip_index, data_length):
...
def test(config, model, queryloader, galleryloader, dataset):
    ...
    cmc, mAP = evaluate_with_locations(distmat, q_pids, g_pids, q_camids, g_camids, mode='SL')
    logger.info("Results ---------------------------------------------------")
    logger.info('top1:{:.1%} top5:{:.1%} top10:{:.1%} top20:{:.1%} mAP:{:.1%}'.format(cmc[0], cmc[4], cmc[9], cmc[19], mAP))
    logger.info("-----------------------------------------------------------")

    logger.info("Computing CMC and mAP only for locations-changing")
    cmc, mAP = evaluate_with_locations(distmat, q_pids, g_pids, q_camids, g_camids, mode='DL')
    logger.info("Results ---------------------------------------------------")
    logger.info('top1:{:.1%} top5:{:.1%} top10:{:.1%} top20:{:.1%} mAP:{:.1%}'.format(cmc[0], cmc[4], cmc[9], cmc[19], mAP))
    logger.info("-----------------------------------------------------------")
    ...
    cmc, mAP = evaluate_with_scales(distmat, q_pids, g_pids, q_camids, g_camids, mode='SS')
    logger.info("Results ---------------------------------------------------")
    logger.info('top1:{:.1%} top5:{:.1%} top10:{:.1%} top20:{:.1%} mAP:{:.1%}'.format(cmc[0], cmc[4], cmc[9], cmc[19], mAP))
    logger.info("-----------------------------------------------------------")

    logger.info("Computing CMC and mAP only for scales-changing")
    cmc, mAP = evaluate_with_scales(distmat, q_pids, g_pids, q_camids, g_camids, mode='DS')
    logger.info("Results ---------------------------------------------------")
    logger.info('top1:{:.1%} top5:{:.1%} top10:{:.1%} top20:{:.1%} mAP:{:.1%}'.format(cmc[0], cmc[4], cmc[9], cmc[19], mAP))
    logger.info("-----------------------------------------------------------")
    return cmc[0]
